...
function buttonClick()
           	{
            	...
            	if(document.getElementById('q_3_a_0').checked)
           		    ...
           		    total+=parseInt(document.getElementById('q_10_a_0').value);
           		else if(document.getElementById('q_10_a_1').checked)
           		    total+=parseInt(document.getElementById('q_10_a_1').value);
                else if(document.getElementById('q_10_a_2').checked)
           		    total+=parseInt(document.getElementById('q_10_a_2').value);
           		else if(document.getElementById('q_10_a_3').checked)
           		    total+=parseInt(document.getElementById('q_10_a_3').value);
           		    
           		               		    if(document.getElementById('q_11_a_0').checked)
           		    total+=parseInt(document.getElementById('q_11_a_0').value);
           		else if(document.getElementById('q_11_a_1').checked)
           		    total+=parseInt(document.getElementById('q_11_a_1').value);
                else if(document.getElementById('q_11_a_2').checked)
           		    total+=parseInt(document.getElementById('q_11_a_2').value);
           		else if(document.getElementById('q_11_a_3').checked)
           		    total+=parseInt(document.getElementById('q_11_a_3').value);
           		   
           		//alert (total);
           		var result;
                if(total>=0 && total <= 3)
           		{
           			result="Time to google,dude!";
           			result+="\n";
           			result+="You should try to learn about things around you.";
           			result+="\n";
           			result+="Then,you'll see how beautiful the world is.";
           			result+="\n";
           			result+="Too much knowledge maketh fool but, less knowledge make you noob!";
           		}
           		else if(total>=4 && total <=7)
           		{
           			result="Good job pal...";
           			result+="\n";
           			result+="You know about the world.... ";
           			result+="\n";
           			result+="But,pls feel free to learn more";
           			result+="\n";
           			result+="I'm sure you will enjoy";}
           			
           		else	if(total>=8 && total <= 10)
           		{
           			result="Wow!!!!";
           			result+="\n";
           			result+="You are a true observer. ";
           			result+="\n";
           			result+="People think you don't notice anything but actually ,you do...";
           			result+="\n";
           			result+="Welldone mate!!";
                  ...
                  else	if(total>=11)
           		{
           			result=" ";
           			result+="\n";
           			result+="For God's Sake, you know everything!!!!";
           			result+="\n";
           			result+="How impressive !!";
           			result+="\n";
           			result+="What a genius you are!";
           		...
           		function validate(form)
{
	...
	if(!checkRadioArray(f1.q_1_a)){
		...
		if(!checkRadioArray(f1.q_10_a)){
		errors[errors.length]="You left No.11 questions!";
			}
		if(!checkRadioArray(f1.q_11_a)){
		errors[errors.length]="You left No.12 questions!";
		}
